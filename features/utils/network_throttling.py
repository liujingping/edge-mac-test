...
class NetworkThrottlingManager:
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['which', 'dnctl'], capture_output=True, text=True)
            if result.returncode != 0:
...
"""
            ...
            result = subprocess.run(
                ['sudo', 'pfctl', '-e'], capture_output=True, text=True
            )
            if result.returncode != 0:
    ...
    def remove_throttling(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ['sudo', 'pfctl', '-a', self.pf_anchor, '-F', 'all'],
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
            ...
            subprocess.run(
                ['sudo', 'dnctl', 'pipe', '1', 'delete'], capture_output=True, text=True
            )
            subprocess.run(
                ['sudo', 'dnctl', 'pipe', '2', 'delete'], capture_output=True, text=True
            )
...
def apply_profile(manager: NetworkThrottlingManager, profile_name: str) -> bool:
    ...
    logger.info(
        f"Applying throttling profile '{profile_name}': {profile['description']}"
    )
