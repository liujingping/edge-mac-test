    ...
    result = call_tool_sync(
        ...
        context.session.call_tool(
            ...
            arguments={
                'caller': 'behave-automation',
                'need_snapshot': 0,
                'task_description': 'Verify the webpage is in reading mode - the page should '
                'show a clean, simplified layout with the Wikipedia '
                'article content displayed in a readable format, with the '
                'reading mode toolbar visible at the top, and the URL '
                "showing 'read://' instead of 'https://'",
            },
        ),
    )
    result_json = get_tool_json(result)
    ...
    result = call_tool_sync(
        ...
        context.session.call_tool(
            ...
            arguments={
                'caller': 'behave-automation',
                'need_snapshot': 0,
                'task_description': 'Verify the webpage has exited from reading mode - the '
                'page should show the normal Wikipedia layout with the '
                'standard header, sidebar, navigation elements, and the '
                "URL should show 'https://' instead of 'read://', with no "
                'reading mode toolbar visible',
            },
        ),
    )
    result_json = get_tool_json(result)
