...
def step_impl(context):
    ...
    try:
        ...
        find_result = call_tool_sync(context, context.session.call_tool(
            name="find_element", 
            arguments={
        ...
        if find_result_json.get("status") == "success":
            print("DEBUG: Found 'Welcome to Microsoft Edge' dialog, clicking 'Start Microsoft Edge' button")
            # Click the "Start Microsoft Edge" button
            click_result = call_tool_sync(context, context.session.call_tool(
                name="click_element",
                arguments={
                    'caller': 'behave-automation',
                    'locator_value': 'Start Microsoft Edge',
            ...
            assert click_result_json.get("status") == "success", f"Failed to click 'Start Microsoft Edge' button: {click_result_json.get('error')}"
            print("DEBUG: Successfully clicked 'Start Microsoft Edge' button")
        else:
            print("DEBUG: No 'Welcome to Microsoft Edge' dialog found, Edge may already be running")
    ...
    try:
        ...
        sync_find_result = call_tool_sync(context, context.session.call_tool(
            name="find_element", 
            arguments={
                'caller': 'behave-automation',
        ...
        if sync_find_result_json.get("status") == "success":
            ...
            sync_click_result = call_tool_sync(context, context.session.call_tool(
                name="click_element",
                arguments={
                    'caller': 'behave-automation',
                    'locator_value': 'Not now',
            ...
            assert sync_click_result_json.get("status") == "success", f"Failed to click 'Not now' button: {sync_click_result_json.get('error')}"
    ...
    print("DEBUG: Edge launch step completed")
